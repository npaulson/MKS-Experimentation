...
f = h5py.File("spatial_trim.hdf5", 'w')
f.close()

"""Trim the statistics for the microstructures"""
for ii in xrange(len(set_id_cal)):
    gt.trim(ns_cal[ii], set_id_cal[ii])
for ii in xrange(len(set_id_val)):
    gt.trim(ns_val[ii], set_id_val[ii])
