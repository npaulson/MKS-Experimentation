...
micr1 = rr.pha_loc("cont5_micr_new48.txt", el, 48)
micr2 = rr.pha_loc("cont5_micr_old.txt", el, 51)
micr = np.concatenate((micr1, micr2),axis=3)
...
if read_dat == 1:
    ...
    for n in xrange(ns):
        if n <= 47:        
            filename = "sq21_%scont5_v2.dat" %(n+2) 
            resp[:,:,:,n] = rr.res_red(filename)
            print "%s is loaded" %filename          
        else:        
            filename = "sq21_%scont5.dat" %(n-47) 
            resp[:,:,:,n] = rr.res_red(filename)
            print "%s is loaded" %filename 
