...
def vtk_write(el, ns, set_id, step, loading, newdir, wrt_file):
    ...
    e11 = np.load('epsilon%s_mks_%s%s_s%s.npy' % (11, ns, set_id, step)).reshape([ns, el**3])
    e22 = np.load('epsilon%s_mks_%s%s_s%s.npy' % (22, ns, set_id, step)).reshape([ns, el**3])
    e33 = np.load('epsilon%s_mks_%s%s_s%s.npy' % (33, ns, set_id, step)).reshape([ns, el**3])
    e12 = 0.5 * np.load('epsilon%s_mks_%s%s_s%s.npy' % (12, ns, set_id, step)).reshape([ns, el**3])
    e13 = 0.5 * np.load('epsilon%s_mks_%s%s_s%s.npy' % (13, ns, set_id, step)).reshape([ns, el**3])
    e23 = 0.5 * np.load('epsilon%s_mks_%s%s_s%s.npy' % (23, ns, set_id, step)).reshape([ns, el**3])

    s11 = np.load('sigma%s_mks_%s%s_s%s.npy' % (11, ns, set_id, step)).reshape([ns, el**3])
    s22 = np.load('sigma%s_mks_%s%s_s%s.npy' % (22, ns, set_id, step)).reshape([ns, el**3])
    s33 = np.load('sigma%s_mks_%s%s_s%s.npy' % (33, ns, set_id, step)).reshape([ns, el**3])
    s12 = np.load('sigma%s_mks_%s%s_s%s.npy' % (12, ns, set_id, step)).reshape([ns, el**3])
    s13 = np.load('sigma%s_mks_%s%s_s%s.npy' % (13, ns, set_id, step)).reshape([ns, el**3])
    s23 = np.load('sigma%s_mks_%s%s_s%s.npy' % (23, ns, set_id, step)).reshape([ns, el**3])
