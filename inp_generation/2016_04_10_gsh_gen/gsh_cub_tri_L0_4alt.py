...
def gsh_eval(X, Bvec):
    ...
    for Bindx in Bvec:
        ...
        if Bindx == 3:
            ...
            t32 = 2 * (-t29 + 1) * t27
            t28 = 2 * phi1
            tfunc[..., c] = (0.3e1 / 0.32e2) * np.sqrt(0.2e1) * np.sqrt(0.5e1) * np.sqrt(0.21e2) * ((14 * t25 - 16 * t29 + 2) * np.exp((-2*1j) * phi2) + (-t32 + t33) * np.exp((-2*1j) * (phi2 + t28)) + (t32 + t33) * np.exp((2*1j) * (t28 - phi2)))
        ...
        if Bindx == 7:
            ...
            t69 = 2 * (-t66 + 1) * t64
            t65 = 2 * phi1
            tfunc[..., c] = (0.3e1 / 0.32e2) * np.sqrt(0.2e1) * np.sqrt(0.5e1) * np.sqrt(0.21e2) * ((14 * t62 - 16 * t66 + 2) * np.exp((2*1j) * phi2) + (t69 + t70) * np.exp((-2*1j) * (t65 - phi2)) + (-t69 + t70) * np.exp((2*1j) * (phi2 + t65)))
