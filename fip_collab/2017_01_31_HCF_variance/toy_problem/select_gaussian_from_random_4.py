...
n_tri = 100
costvec = np.zeros((n_tri,))
iimat = np.zeros((n_tri, n_tgt), dtype='int16')

for jj in xrange(n_tri):
    newpts = np.zeros((n_tgt, 2))
    rawdata_ = rawdata
    iivec = np.int16(np.arange(n_tgt))
    np.random.shuffle(iivec)
    iimat[jj, :] = iivec
    for ii in iivec:
        dist = np.sqrt(np.sum((rawdata_-targets[ii, :])**2, 1))
        indx = np.argmin(dist)
        costvec[jj] += dist[indx]**2

        newpts[ii, :] = rawdata_[indx, :]
        rawdata_ = np.delete(rawdata_, indx, axis=0)

print "minimum cost: " + str(costvec.min())
print "maximum cost: " + str(costvec.max())
indx = np.argmax(costvec)

newpts = np.zeros((n_tgt, 2))
rawdata_ = rawdata
for ii in iimat[indx, :]:
    dist = np.sqrt(np.sum((rawdata_-targets[ii, :])**2, 1))
    indx = np.argmin(dist)
