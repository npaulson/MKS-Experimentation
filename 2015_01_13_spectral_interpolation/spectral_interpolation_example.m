...
scatter(0:Nd-1,real(f_d_fft));
title('Real frequencies in the original FFT')
xlabel('frequency')
ylabel('amplitude')
axis([0 Nd-1 minB(real(f_d_fft)) maxB(real(f_d_fft))])

subplot(2,1,2)

scatter(0:Nd-1,imag(f_d_fft));
title('Imaginary frequencies in the original FFT')
xlabel('frequency')
ylabel('amplitude')
axis([0 Nd-1 minB(imag(f_d_fft)) maxB(imag(f_d_fft))])

% zero pad in frequency space

scale = 2;

f_Nd_fft = zeros(1,scale*Nd);
...
% f_Nd_fft(end - 0.5*Nd + 1: end) = f_d_fft(0.5*Nd+1:end);
...
scatter(x_d_new,real(f_eval_interpolate(1:end-1)),'rx')
