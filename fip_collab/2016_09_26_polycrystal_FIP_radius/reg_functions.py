...
def prediction(x, coef, n_pc, deg):
    """compute the data matrices with dimensions
    n_samp x n_feat where the features contain all polynomial terms
    up to the specified degree"""
...
def loocv(x, y, n_pc, deg):
    ...
    for ii in xrange(y.size):
        ...
        coef = regression(x[tmp, :], y[tmp], n_pc, deg)
        ypred = prediction(x[tmp_inv, :], coef, n_pc, deg)
...
def loocv_set(x, y, n_pc, deg, p):

    n_cv = np.int32(np.round(y.size/np.float32(p)))
    print n_cv
    print p

    err_raw = np.zeros((n_cv*p,))

    c = 0

    for ii in xrange(n_cv):
        ...
        err_raw[c:c_] = np.abs(y[tmp_inv]-ypred)
...
def standard(x, y, n_pc, deg):

    C = const()

    coef = regression(x, y, n_pc, deg)

    y_predict = prediction(x, coef, n_pc, deg)
