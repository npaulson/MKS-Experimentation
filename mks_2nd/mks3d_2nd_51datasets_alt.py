...
micr = rr.gen_micr('M_secondorder.mat', ns, el)
...
M = np.zeros((el, el, el, ns, H),dtype = 'complex128')# + 0j*np.zeros((el, el, el, ns, H))
for n in xrange(ns):
    for h in xrange(H):
...
# saved FE_results.npy
    ...
    resp = np.load('FE_results_51.npy')
