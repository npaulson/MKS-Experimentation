...
def add_samp(M_k,resp_fft_k, MM, PM):
    """
    Summary:    
    Inputs:
        M_k: This should have the form M[u,v,w,:,:]
        resp_fft_k: This should have the form resp_fft[u,v,w,:]
        MM:
        PM:
    Outputs:
        PM: PM with the addition of new samples
        MM: MM with the addition of new samples
    """
    for n in xrange(len(M_k[0,:])):
        mSQ = np.array(M_k[n,:])     
        mSQc = np.conj(mSQ[None,:])
        mSQt = mSQ[:,None]        
        MM = MM + np.dot(mSQt, mSQc)
        PM[:,0] = PM[:,0] + np.dot(resp_fft_k[n],mSQc)   

    return [MM, PM]

def eval_meas(mks_R, resp_fin, el=21):
    """
    Summary:
    Inputs:
    ...
    for k in xrange(el**3):
        [u,v,w] = np.unravel_index(k,[el,el,el])
        MASE = MASE + ((np.abs(resp_fin[u,v,w] - mks_R[u,v,w]))/(avgE * el**3))
