    ...
    for l in xrange(1, kmax):
        ...
        for m in xrange(1, kmax):
            for n in xrange(1, kmax):
                ...
                tmp1 = ar_fft[l, m, n] * np.exp((2*np.pi*1j*l*xi[0])/L) + \
                    ar_fft[N-l, N-m, N-n] * np.exp((-2*np.pi*1j*l*xi[0])/L)

                tmp2 = ar_fft[l, m, n] * np.exp((2*np.pi*1j*m*xi[0])/L) + \
                    ar_fft[N-l, N-m, N-n] * np.exp((-2*np.pi*1j*m*xi[0])/L)

                tmp3 = ar_fft[l, m, n] * np.exp((2*np.pi*1j*n*xi[0])/L) + \
                    ar_fft[N-l, N-m, N-n] * np.exp((-2*np.pi*1j*n*xi[0])/L)
