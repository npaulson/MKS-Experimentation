...
def euler_rand(n_tot, phi1max, phimax, phi2max):
    ...
    phi1 = np.random.rand(n_tot)*phi1max*deg2rad
    phi = np.random.rand(n_tot)*phimax*deg2rad
    phi2 = np.random.rand(n_tot)*phi2max*deg2rad
...
for ii in xrange(N_L):
    ...
    c_tot = c_eul*bsz_eul
    tmp = c_tot*np.sum(Y*X[:, ii].conj()*np.sin(euler[:, 1]))
...
# X is the array of basis functions evaluated at the validation data
# points
X = gsh.gsh_eval(euler, np.arange(N_L))
