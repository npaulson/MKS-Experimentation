...
the script saves the coefficients for the legendre polynomial

the script also records the largest detected error 
"""
...
for ii in xrange(0, n_tot):
    ...
    if np.max(error) > maxerr:
        ...
        np.save("ep11_%s.npy" % str(tnum[1]).zfill(2), ep11)
        np.save("et_norm_%s.npy" % str(tnum[1]).zfill(2), et_norm)
        np.save("coeff_set_%s.npy" % str(tnum[1]).zfill(2), coeff_set)
        # break
    # print euler[ii, :]
    # print np.mean(error)
    # print np.max(error)
