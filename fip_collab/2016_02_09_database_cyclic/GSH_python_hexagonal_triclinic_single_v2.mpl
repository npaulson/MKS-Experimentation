...
return tfunction_final, index_final:
...
end_l := 4:
l_set := [0,1,2,3,4]:
tfunction_final, index_final := genTFunction(l_set):

#mmm := evalf(eval(Matrix(tfunction_final), [phi1 = .1, phi = .2, phi2 = .3]));
print(Matrix(index_final)):
print(Matrix(tfunction_final)):

# begin to format the beginning of the python file
import1 := "import numpy as np\n\n\n":
funchead1 := "def gsh_basis_info():\n\n":

python_output := cat(import1, funchead1):

python_output := cat(python_output, "    indxvec = np.array(["):

tmp := convert(index_final[1][1], list):
python_output := cat(python_output, "[", tmp[1], ", ", tmp[2], ", ", tmp[3], "]"):

for indx from 2 to Size(Matrix(index_final),1)
do

tmp := convert(index_final[indx][1], list):
python_output := cat(python_output, ",\n                        [", tmp[1], ", ", tmp[2], ", ", tmp[3], "]"):
...
# simplify the GSH functions
tfunction_sim := simplify(Matrix(tfunction_final)):

for indx from 1 to Size(tfunction_sim)[1]
do

python_output := cat(python_output, "        if Bindx == ", indx-1, ":\n"):

# optimize the GSH functions so that common subexpressions can be evaluated only once
sing_func_opt := eval([codegen:-optimize](tfunction_sim[indx], tryhard), pow= `^`):

for indx2 from 1 to Size(sing_func_opt)[2]-1
do

temp_str := Matlab([sing_func_opt[indx2]], output=string):
#print(temp_str):

python_output := cat(python_output, "            ", temp_str):

end do:

temp_str := Matlab(rhs(sing_func_opt[-1])[1], resultname = "blarg", output=string):
python_output := cat(python_output, "            ", temp_str, "\n"):
...
python_output := cat(python_output, autorun_str):

python_output := SubstituteAll(python_output, "^", "**"):
python_output := SubstituteAll(python_output, "(i)", "(1j)"):
python_output := SubstituteAll(python_output, "*i", "*1j"):
python_output := SubstituteAll(python_output, "exp", "np.exp"):
python_output := SubstituteAll(python_output, "sqrt", "np.sqrt"):
python_output := SubstituteAll(python_output, "cos", "np.cos"):
python_output := SubstituteAll(python_output, "sin", "np.sin"):
python_output := SubstituteAll(python_output, ";", ""):
python_output := SubstituteAll(python_output, "blarg", "tfunc[..., c]"):

file := cat("C:/Users/nhpnp3/Documents/MKS-Experimentation/inp_generation/2015_11_12_gsh_gen/gsh_hex_tri_L",start_l,"_",end_l,".py"):
fd := fopen(file, WRITE):
fprintf(fd, "%s\n", python_output):
fclose(fd):
print(python_output):
