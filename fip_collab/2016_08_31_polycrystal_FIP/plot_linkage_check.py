...
def plot_check(par, n_pc, n_poly, H, erv):
    ...
    c = 0
    for ii in xrange(len(C['ns_cal_split'])):

        # name = C['names_cal'][ii]
        Rsim_tmp = RsimC[ii]
        Rpred_tmp = RpredC[ii]

        plt.text(Rsim_tmp, Rpred_tmp+0.05*valrange, C['names_cal_split'][ii],
    ...
    for ii in xrange(len(C['ns_val_split'])):

        # name = C['names_val'][ii]
        Rsim_tmp = RsimV[ii]
        Rpred_tmp = RpredV[ii]

        plt.text(Rsim_tmp, Rpred_tmp+0.05*valrange, C['names_val_split'][ii],
                 horizontalalignment='center',
                 verticalalignment='center')
