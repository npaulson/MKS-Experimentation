...
def plterr(par, upbnd, deg, Tvec, Hvec):
    ...
    for ii in xrange(len(Hvec)):
        for jj in xrange(len(Tvec)):
            ...
            if Tvec[jj] == 'cal':
                ...
                label = 'LOOCV, L=%s'
                tmp = f.get('LOOCV_%s' % par)[isdeg, :]
                err_tmp = np.mean(tmp, 1)

            else:
                label = 'null'
                err_tmp = np.zeros((C['n_pc_max'],))

            err = 100*np.abs(err_tmp/sim.mean())

            plt.plot(np.arange(C['n_pc_max'])+1, err,
                     marker='', markersize=8,
    ...
    typs = ''.join(Tvec)
    fig_name = 'selection_%s_%s_deg%s_npc%s.png' % (typs, par, deg, C['n_pc_max'])
