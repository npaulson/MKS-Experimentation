...
% vol_frac1 = 0.5;
% sigma = logspace(log10(1),log10(7),20);
% standard_dev = linspace(-1.5,1.5,21);
% 
% for vf1 = vol_frac1 
% for sig = sigma
% for st = standard_dev
%     tmp = single(rand(el,el,el) > vf1);
%     tmp = imgaussfilt3(tmp,sig,'Padding','circular');
%     max_v = max(tmp(:));
%     min_v = min(tmp(:));
%     tmp = (tmp - min_v)/(max_v-min_v);
%     tmp = reshape(tmp,[1,el^3]);
%     M(c,:) = tmp > mean(tmp(:))+st*std(tmp(:));
%     c = c + 1;
% end 
% end
% end
...
pdS = 10;
mask = padarray(ones(el,el,el),[pdS,pdS,pdS]);
denom = FullConv('a','n','m', mask, mask);
...
for ii = 1:ns
    ...
    tmp_2pt = numer./denom;
    tmp_2pt = tmp_2pt(3*pdS+1:end-3*pdS,...
                      3*pdS+1:end-3*pdS,...
                      3*pdS+1:end-3*pdS);
    M_2pt(ii,:) = reshape(tmp_2pt,[1,el^3]);
...
[COEFF,SCORE,latent,tsquared,explained,mu] = pca(M_2pt);
...
tmp_2pt = tmp_2pt(3*pdS+1:end-3*pdS,...
                  3*pdS+1:end-3*pdS,...
                  3*pdS+1:end-3*pdS);
m_st_2pt = reshape(tmp_2pt,[1,el^3]);
...
%     pdN = 1;
    ...
    idxX = round(rand()*((el)-1)+1)+pdN;
    idxY = round(rand()*((el)-1)+1)+pdN;
    idxZ = round(rand()*((el)-1)+1)+pdN;
    
    % change the microstructure into 3D format
    tmp = reshape(m_pbN,[el,el,el]);    
    
    tmp = padarray(tmp,[pdN,pdN,pdN]);
    tmp(idxX-pdN:idxX+pdN,idxY-pdN:idxY+pdN,idxZ-pdN:idxZ+pdN) = rand()>.5;
    tmp = tmp(pdN+1:end-pdN,pdN+1:end-pdN,pdN+1:end-pdN);
    ...
    tmp_2pt = tmp_2pt(3*pdS+1:end-3*pdS,...
                      3*pdS+1:end-3*pdS,...
                      3*pdS+1:end-3*pdS);
    m_pb_2pt = reshape(tmp_2pt,[1,el^3]);
    m_pbN_pc = (m_pb_2pt - M_2pt_mean) * COEFF;
    
    
    diffO = m_pbO_pc(2:7) - m_st_pc(2:7);
    diffN = m_pbN_pc(2:7) - m_st_pc(2:7);
    distO = sqrt(diffO*diffO');
    distN = sqrt(diffN*diffN');
    
    lvl = 1 - ((Nmax-ii)/Nmax)*rand();
...
legend('deltas','halves','randoms','halves2')
xlabel(['PC',int2str(pc_A)]);
ylabel(['PC',int2str(pc_B)]);
zlabel(['PC',int2str(pc_C)]);
axis tight;
