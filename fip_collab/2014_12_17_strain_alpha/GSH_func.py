...
def gsh_hcp_tri_L_4(e_angles):
    ...
    out_tvalues = np.zeros([e_angles.shape[0],15], dtype = 'complex128')    
...
def GSH_Hexagonal_Triclinic(e_angles):
    ...
    Tsym = np.zeros([e_angles.shape[0],15], dtype = 'complex128')    
...
if __name__ == '__main__':
    tvals = GSH_Hexagonal_Triclinic(np.array([[0.1,0.2+0.5*np.pi,0.3],[.1,0,.3],[100,200,300]]))
    print tvals
