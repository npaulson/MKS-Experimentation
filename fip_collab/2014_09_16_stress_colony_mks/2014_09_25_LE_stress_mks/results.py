...
def results_all(ns,set_id,typ):
    ...
    real_comp_desig = ['11','22','33','12','13','23','vm']    
    
    mks_R = np.zeros([el,el,el,7,ns])
    resp = np.zeros([el,el,el,7,ns])      
    ...
    mks_R[:,:,:,6,:] = np.sqrt( 0.5*( (mks_R[:,:,:,0,:]-mks_R[:,:,:,1,:])**2 +(mks_R[:,:,:,1,:]-mks_R[:,:,:,2,:])**2 + (mks_R[:,:,:,2,:]-mks_R[:,:,:,0,:])**2 + 6*(mks_R[:,:,:,3,:]**2 + mks_R[:,:,:,4,:]**2 + mks_R[:,:,:,5,:]**2) ) )

    mean_resp_vm = np.mean(resp[:,:,:,6,:])
    
    for comp in xrange(7):
