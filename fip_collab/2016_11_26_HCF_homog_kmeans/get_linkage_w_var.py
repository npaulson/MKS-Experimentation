...
"""
In this version of the code subsamples are represented
by both the mean location and the standard deviation
in each considered PC dimension
"""
...
def linkage(par):
    ...
    reduced_tot = np.zeros((ns_tot, 2*C['n_pc_tot']), dtype='float64')
    ...
    for ii in xrange(n_tot):
        ...
        reduced_tot[c:c_, ::2] = means
        reduced_tot[c:c_, 1::2] = stds

        # for jj in xrange(C['n_pc_tot']):
        #     reduced_tot[c:c_, 2*jj] = means[:, jj]
        #     reduced_tot[c:c_, 2*jj+1] = stds[:, jj]
    ...
    for ii in xrange(n_ii):
        for jj in xrange(n_jj):
            ...
            loocv_err = rf.loocv(reduced_tot[iscal, :(2*n_pc)],
                                 response_tot[iscal],
                                 deg)

            coef = rf.regression(reduced_tot[iscal, :(2*n_pc)],
                                 response_tot[iscal],
                                 deg)

            pred = rf.prediction(reduced_tot[:, :(2*n_pc)],
                                 coef,
                                 deg)
