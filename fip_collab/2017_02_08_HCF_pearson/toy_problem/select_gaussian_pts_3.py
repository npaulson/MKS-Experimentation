...
for ii in xrange(500):
    ...
    with min error"""
    tmpmax = np.argmin(np.abs(sel-bin_centers[np.argmax(err)]))
    tmpmin = np.argmin(np.abs(X-bin_centers[np.argmax(err)]))
    sel[tmpmax] = X[tmpmin]

    X = np.delete(X, tmpmin)

    hist, bin_edges = np.histogram(sel, bins=50,
                                   range=(xmin, xmax), density=True)
    binw = bin_edges[1] - bin_edges[0]
    bin_centers = bin_edges[:-1]+0.5*binw

plt.plot(bin_centers, hist, 'k-', alpha=0.3)
