...
def linkage(prop, bc):
    ...
    for ii in xrange(len(C['ns_cal'])):
        ...
        dset_name = "%s_%s_%s" % (prop, bc, set_id)
    ...
    Rpred_cal_set = f_reg.create_dataset('Rpred_cal_%s_%s' % (prop, bc),
                                         (n_ii*n_jj, ns_cal_tot),
                                         dtype='float64')

    Rpred_val_set = f_reg.create_dataset('Rpred_val_%s_%s' % (prop, bc),
                                         (n_ii*n_jj, ns_val_tot),
                                         dtype='float64')

    f_reg.create_dataset('Rsim_cal_%s_%s' % (prop, bc), data=response_cal_tot)

    f_reg.create_dataset('Rsim_val_%s_%s' % (prop, bc), data=response_val_tot)

    order_set = f_reg.create_dataset('order',
                                     (n_ii*n_jj, 2),
                                     dtype='int64')

    meanerr_cal_set = f_reg.create_dataset('meanerr_cal_%s_%s' % (prop, bc),
                                           (n_ii*n_jj,),
                                           dtype='float64')

    meanerr_val_set = f_reg.create_dataset('meanerr_val_%s_%s' % (prop, bc),
                                           (n_ii*n_jj,),
                                           dtype='float64')

    maxerr_cal_set = f_reg.create_dataset('maxerr_cal_%s_%s' % (prop, bc),
                                          (n_ii*n_jj,),
                                          dtype='float64')

    maxerr_val_set = f_reg.create_dataset('maxerr_val_%s_%s' % (prop, bc),
                                          (n_ii*n_jj,),
                                          dtype='float64')

    coef_set = f_reg.create_dataset('coef_%s_%s' % (prop, bc),
                                    (n_ii*n_jj, coefmax),
                                    dtype='float64')
