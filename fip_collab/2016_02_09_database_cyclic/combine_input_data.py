...
alldata = f1.create_dataset("var_set", (n_par*n_th*n_en, 6))

c = 0

for tt in xrange(0, n_th*n_en):

    en_indx = np.mod(tt, n_en)
    th_indx = np.floor(tt/n_en)
    print "en_indx: %s" % en_indx
    print "Deformation Mode: %s deg" % str((th_indx+0.5)*inc_th)
