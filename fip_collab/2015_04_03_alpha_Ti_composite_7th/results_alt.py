...
def results(el, ns, set_id, typ, wrt_file):
    ...
    fe = np.max(r_fem.reshape(ns, el**3), 1)*100
    ...
    # for sn in xrange(ns):
    #     avgr_fe_indv = np.average(r_fem[sn, ...])
    #     avgr_mks_indv = np.average(r_mks[sn, ...])
    ...
    #     % (typ, real_comp, np.std(r_fem))
    # rr.WP(msg, wrt_file)
    # msg = 'Standard deviation, %s%s, MKS: %s' \
    #     % (typ, real_comp, np.std(r_mks))
    # rr.WP(msg, wrt_file)
    ...
    # msg = 'Mean minimum, %s%s, FEM: %s' % (typ, real_comp, r_fem_min)
    # rr.WP(msg, wrt_file)
    # msg = 'Mean minimum, %s%s, MKS: %s' % (typ, real_comp, r_mks_min)
    # rr.WP(msg, wrt_file)
    # msg = 'Mean maximum, %s%s, FEM: %s' % (typ, real_comp, r_fem_max)
    # rr.WP(msg, wrt_file)
    # msg = 'Mean maximum, %s%s, MKS: %s' % (typ, real_comp, r_mks_max)
    # rr.WP(msg, wrt_file)
