...
def validation_procedure(ns_cal,ns_val,set_id_cal,set_id_val,comp,wrt_file):
    ...
    for sn in xrange(ns_val):
        specinfc_sqr = np.reshape(specinfc,[el,el,el,H])
        mks_F = np.sum(np.conjugate(specinfc_sqr) * M[:,:,:,sn,:],3)
        mks_R[:,:,:,sn] = np.fft.ifftn(mks_F).real

        
    np.save('mksR%s_%s%s' %(comp,ns_val,set_id_val), mks_R)
