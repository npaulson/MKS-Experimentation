...
f = h5py.File('var_extract_th60_6deg_fip.hdf5', 'r')
...
# note that these angular variables are rescaled between -1 and 1
phi1_c = real2comm(phi1, 0, phi1max)
phi_c = real2comm(phi, 0, phimax)
phi2_c = real2comm(phi2, 0, phi2max)
...
for I in xrange(ImatL):
    ...
    if np.mod(I, ImatL/10) == 0:
        print "%s%% complete" % np.round(100*I/ImatL, 0)
