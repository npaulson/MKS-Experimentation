...
#        specinfc_pad[:,:,:,h] = edgeHvec[h]
    ...
    for ii in xrange(setnum):
        ...
        ss = ii*setsz  # sn for start of set

        if ii == setnum - 1:
            se = ns_val
        else:
            se = (ii+1)*setsz  # sn for end of set

        M = np.load('M_%s%s_%s-%s.npy' % (ns_val, set_id_val, ss, se))

        tmp = np.sum(np.conjugate(specinfc) * M, 1)
        mks_R[ss:se+1, ...] = np.fft.ifftn(tmp, [el_val, el_val, el_val],
                                           [1, 2, 3]).real
