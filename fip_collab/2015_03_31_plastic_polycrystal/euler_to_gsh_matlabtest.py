...
def euler_to_gsh(el, H, ns, set_id, step, wrt_file):
    ...
    # call the desired GSH function in matlab
    tmp = eng.gsh_hex_tri_L0_4(matlab.double(list(euler[:, 0, :].reshape(ns*el**3))),
                               matlab.double(list(euler[:, 1, :].reshape(ns*el**3))),
                               matlab.double(list(euler[:, 2, :].reshape(ns*el**3))))

    # convert the result back to a numpy array and store
    euler_GSH = np.array(tmp).reshape(ns, H, el**3)
