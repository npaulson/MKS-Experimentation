...
import get_correlation_average as gca
...
"""Take the averaged statistics"""
for ii in xrange(len(set_id_cal)):
    gca.avg(ns_cal[ii], set_id_cal[ii])
for ii in xrange(len(set_id_cal)):
    gca.avg(ns_val[ii], set_id_val[ii])

"""Perform PCA on correlations"""
pca = gns.new_space(set_id_cal)
...
for ii in xrange(len(set_id_cal)):
    tf.transform(set_id_cal[ii], pca)
for ii in xrange(len(set_id_val)):
    tf.transform(set_id_val[ii], pca)
