...
def results_all(el,ns,set_id_old,set_id_new,step,typ):

    ## specify the file to write messages to 
    wrt_file = 'results_all_step%s_%s%s_%s.txt' %(step,ns,set_id_new,time.strftime("%Y-%m-%d_h%Hm%M")) 
    ...
    for comp in xrange(10):
        ...
        plt.savefig('field_step%s_comp%s_%s%s.png' %(step,comp,ns,set_id_new))                
        ...
        plt.savefig('hist_step%s_comp%s_%s%s.png' %(step,comp,ns,set_id_new))                



if __name__ == '__main__':
    results_all(25,1,'val25el',1,'sigma')
