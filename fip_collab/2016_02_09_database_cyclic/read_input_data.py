...
xnode = et_norm[sample_indx]  # xnode: en values for nodes
print xnode

nvec = np.array([n_th, n_p1, n_P, n_p2, n_en])
print "nvec: %s" % str(nvec)

en_indx = np.mod(np.float(tnum), n_en)
en_val = xnode[en_indx]
print "en_val: %s" % str(en_val)

th_indx = np.floor(np.float(tnum)/n_en)
th_val = (th_indx+0.5)*sub2rad_th
