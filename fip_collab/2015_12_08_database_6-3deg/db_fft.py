...
tmp = np.roll(chunk, n_hlf, 1)  # roll 180 degrees in phi1
tmp = tmp[:, :, ::-1, :, :]  # flip in Phi
tmp_red = tmp[:, :, :, 1:, :]
tmp_red = tmp_red[:, :, :, ::-1, :]  # flip in phi2
tmp[:, :, :, 1:, :] = tmp_red
...
tmp = pre_fft[:n_th, :, :n_hlf+1, :n_p2, :]  # get section of interest
tmp = tmp[:, :, ::-1, :, :]  # flip in Phi
