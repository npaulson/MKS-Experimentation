...
def results_all(el, ns, set_id, step, typ, compl, comp_app):
    ...
    for comp in xrange(6):
        ...
        ### DIFFERENCE MEASURES ###
        mean_diff_meas = np.mean(abs(r_fem[:,comp,...]-r_mks[:,comp,...]))/app_resp
        mean_max_diff_meas = np.mean(max_diff_all)/app_resp
        max_diff_meas_all = np.amax(abs(r_fem[:,comp,...]-r_mks[:,comp,...]))/app_resp
        ...
        rr.WP(msg,wrt_file)
        msg = 'Standard deviation, %s%s, CPFEM: %s' %(typ,ccur,np.std(r_fem[:,comp,...]))
        rr.WP(msg,wrt_file)
        msg = 'Standard deviation, %s%s, MKS: %s' %(typ,ccur,np.std(r_mks[:,comp,...]))
        rr.WP(msg,wrt_file)

        resp_min = np.mean(np.amin(np.reshape(r_fem[:,comp,...],[ns,el**3]), axis=1))
        r_mks_min = np.mean(np.amin(np.reshape(r_mks[:,comp,...],[ns,el**3]), axis=1))

        resp_max = np.mean(np.amax(np.reshape(r_fem[:,comp,...],[ns,el**3]), axis=1))
