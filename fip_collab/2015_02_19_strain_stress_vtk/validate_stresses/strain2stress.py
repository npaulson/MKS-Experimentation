...
def strain2stress(el, ns, set_id, step, wrt_file):
    ...
    tmp = np.load('epsilon%s_mks_%s%s_s%s.npy' % ('11', ns, set_id, step))
    strain_s[:, :, 0, 0] = tmp.reshape([ns, el**3])
    tmp = np.load('epsilon%s_mks_%s%s_s%s.npy' % ('22', ns, set_id, step))
    strain_s[:, :, 1, 1] = tmp.reshape([ns, el**3])
    tmp = np.load('epsilon%s_mks_%s%s_s%s.npy' % ('33', ns, set_id, step))
    strain_s[:, :, 2, 2] = tmp.reshape([ns, el**3])
    tmp = np.load('epsilon%s_mks_%s%s_s%s.npy' % ('12', ns, set_id, step))
    strain_s[:, :, 0, 1] = 0.5*tmp.reshape([ns, el**3])
    strain_s[:, :, 1, 0] = strain_s[:, :, 0, 1]
    tmp = np.load('epsilon%s_mks_%s%s_s%s.npy' % ('13', ns, set_id, step))
    strain_s[:, :, 0, 2] = 0.5*tmp.reshape([ns, el**3])
    strain_s[:, :, 2, 0] = strain_s[:, :, 0, 2]
    tmp = np.load('epsilon%s_mks_%s%s_s%s.npy' % ('23', ns, set_id, step))
    ...
    np.save('sigma%s_mks_%s%s_s%s.npy' % ('11', ns, set_id, step),
            stress_s[..., 0, 0].reshape([ns, el, el, el]))
    np.save('sigma%s_mks_%s%s_s%s.npy' % ('22', ns, set_id, step),
            stress_s[..., 1, 1].reshape([ns, el, el, el]))
    np.save('sigma%s_mks_%s%s_s%s.npy' % ('33', ns, set_id, step),
            stress_s[..., 2, 2].reshape([ns, el, el, el]))
    np.save('sigma%s_mks_%s%s_s%s.npy' % ('12', ns, set_id, step),
            stress_s[..., 0, 1].reshape([ns, el, el, el]))
    np.save('sigma%s_mks_%s%s_s%s.npy' % ('13', ns, set_id, step),
            stress_s[..., 0, 2].reshape([ns, el, el, el]))
    np.save('sigma%s_mks_%s%s_s%s.npy' % ('23', ns, set_id, step),
            stress_s[..., 1, 2].reshape([ns, el, el, el]))
