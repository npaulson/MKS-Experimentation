...
#        specinfc_pad[:,:,:,h] = edgeHvec[h]
    ...
    specinfc2 = np.zeros([el_cal,el_cal,el_cal,H],dtype = 'complex64')
    for h in xrange(H):
        specinfc2[:,:,:,h] = np.fft.ifftn(np.reshape(pre_specinfc2[:,h],[el_cal,el_cal,el_cal]))

    a = [3,3,3]
    print specinfc_pad[a[0],a[1],a[2],:]
