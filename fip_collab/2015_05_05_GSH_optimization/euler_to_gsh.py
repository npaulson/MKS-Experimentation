...
import numpy as np
import GSH_func_L4 as gsh
import functions as rr
import time
import tables as tb


def euler_to_gsh(el, H, H_, ns, ii, set_id, step, wrt_file):
    ...
    euler_GSH = np.zeros([ns, H_, el**3], dtype='float64')

    for sn in xrange(ns):
        tmp = gsh.gsh(euler[sn, :, :])

        tmp2 = np.zeros([H, el**3], dtype='float64')
    ...
    euler_GSH = euler_GSH.reshape([ns, H_, el, el, el])
