...
def results(infl_coef, y_fem, y_mks, el, ns):

    # specify the file to write messages to
    wrt_file = 'results.txt'

    nfac = np.mean(y_fem)

    error_calc(el, ns, y_fem, y_mks, 'epsilon', '11', 't', nfac, wrt_file)

    maxindx = np.unravel_index(np.argmax(np.abs(y_fem - y_mks)), y_fem.shape)
    maxresp = y_fem[maxindx]
    maxMKS = y_mks[maxindx]
    maxdiff = (np.abs(y_fem - y_mks)[maxindx])
    ...
    field_std(el, ns, y_fem, y_mks, 'epsilon', '11', 't', sn, slc, 1)
    plot_infl_coef(infl_coef, el, 15, 2)
...
def error_calc(el, ns, r_fem, r_mks, typ, comp, spr, nfac, wrt_file):
    ...
    for sn in xrange(ns):
        max_diff_all[sn] = np.amax(np.abs(r_fem[sn, ...]-r_mks[sn, ...]))

    # DIFFERENCE MEASURES
    mean_diff_meas = np.mean(np.abs(r_fem-r_mks))/nfac
    std_diff_meas = np.std(np.abs(r_fem-r_mks))/nfac
    mean_max_diff_meas = np.mean(max_diff_all)/nfac
    max_diff_meas_all = np.amax(np.abs(r_fem-r_mks))/nfac
...
def field_std(el, ns, r_fem, r_mks, typ, comp, spr, sn, slc, plotnum):
...
def plot_infl_coef(infl_coef, el, H, plotnum):

    infl_coef = np.fft.ifftn(infl_coef, axes=(0,1,2))
    infl_coef = np.fft.ifftshift(infl_coef).real

    # Plot slices of the response
    plt.figure(num=plotnum, figsize=[9, 2.7])

    dmin = np.min(infl_coef[:, :, 10, :])
    dmax = np.max(infl_coef[:, :, 10, :])

    plot_size = int(np.sqrt(H))+1
    for c in xrange(1, H+1):
        pltloc = (plot_size,plot_size,c)

        plt.subplot(*pltloc)
        ax = plt.imshow(infl_coef[:, :, 10, c], origin='lower',
                        interpolation='none', cmap='jet', vmin=dmin, vmax=dmax)
        plt.colorbar(ax)
        plt.title('c# %s' % (c-1))
