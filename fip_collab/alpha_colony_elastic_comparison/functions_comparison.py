...
def data_gen(C_CPFEM,C_LE,cyc,comp,comp_latex,SA_lvl,typ,st_comp,wrt_file):
### VISUAL COMPARISON OF CPFEM AND LE+PY SIMULATIONS ###
    ...
    plt.savefig('Field_E%s_cyc%s_%s_%s' %(comp,cyc,SA_lvl,typ))
    ...
    plt.title("Frequency comparison of %s in CPFEM and LE (cycle %s, %s grains, %s%% SA)" %(comp_latex,cyc,typ,(float(SA_lvl)/100.0)))
    
    plt.savefig('Hist_E%s_cyc%s_%s_%s' %(comp,cyc,SA_lvl,typ))
