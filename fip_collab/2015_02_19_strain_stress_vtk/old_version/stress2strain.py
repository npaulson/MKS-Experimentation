...
# Modified by Noah Paulson on February 19,2015
...
strn_t00_el = np.zeros((elements, 2*NumCycles + 1))
strn_t11_el = np.zeros((elements, 2*NumCycles + 1))
strn_t22_el = np.zeros((elements, 2*NumCycles + 1))

for i in range(0, elements):
    R = Gmatrix(euler[i, 0], euler[i, 1], euler[i, 2]).T

    for count in range(0, 2 * NumCycles):
        strain_0 = Bmatrix(strn_t00_el[i, count],
                           strn_t11_el[i, count],
                           strn_t22_el[i, count],
                           strn_t01_el[i, count],
                           strn_t02_el[i, count],
