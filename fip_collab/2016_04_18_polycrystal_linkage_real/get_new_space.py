...
def new_space(el, ns_set, H, set_id_set, step, n_pc_tot, wrt_file):
    ...
    for ii in xrange(len(set_id_set)):
        ...
        print ff.shape
        print np.hstack([ff.real, ff.imag]).shape

        allcorr[c:c+ns_set[ii], ...] = np.hstack([ff.real, ff.imag])
